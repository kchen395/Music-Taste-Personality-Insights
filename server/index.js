...
app.get('/lyrics', function(req, res) {
	...
	// 				'raw_scores': true
	// 			};
	// 			axios.defaults.headers.get['Authorization'] = 'Bearer ' + spotifyToken; 
	...
	instance.get("https://api.spotify.com/v1/me/top/tracks")
		...
		.then((items) => {
			...
			var helper = (item) => {
				...
				.then((data) => {
					if (data.hits[0] && data.hits[0].result.primary_artist.name === artist) return data.hits[0].result.url;
				})
			}
				...
				Promise.all(promises2).then((result) => {
					...
					personalityInsights.profile(profileParams, function(error, profile) {
						if (error) {
							...
							res.status(200).send([personality, songs])
						}
					});
				});
			});
